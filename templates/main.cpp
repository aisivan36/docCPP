#include <iostream>
#include <ostream>
#include <string>

template <typename T> T min(T a, T b) { return (a < b) ? a : b; }

template <typename T1, typename T2> void func(T1 a, T2 b) {
  std::cout << a << b << '\n';
}

struct Person {
  std::string name;
  int age;
  bool operator<(const Person &rhs) const { return this->age < rhs.age; }
};

std::ostream &operator<<(std::ostream &os, const Person &ref) {
  os << ref.name;
  return os;
}

// Swap a value with template
template <typename T> void my_swap(T &a, T &b) {
  T temp = a;
  a = b;
  b = temp;
}

int main() {

  // Call swap
  std::string x{"Scotish"};
  std::string y{"Swedish"};
  std::cout << x << ", and " << y << std::endl;
  my_swap(x, y);
  std::cout << x << ", and " << y << std::endl;
  // The end of swap

  Person p1{"Curly ", 20};
  Person p2{"Tella ", 40};

  Person p3 = min(p1, p2);
  std::cout << p3.name << p3.age << " is younger" << std::endl;

  func(p1, p2);

  std::cout << min<int>(2, 3) << std::endl;
  std::cout << min(2, 3) << std::endl;
  std::cout << min('A', 'B') << std::endl;
  std::cout << min(12.5, 9.2) << std::endl;
  std::cout << min(5 + 2 * 2, 7 + 40) << std::endl;

  func<int, int>(10, 20);
  func(10, 20);
  func<char, double>('A', 30.5);
  func('A', 57.3);
  func(1000, " T2 b whatever");
  func(2000, std::string{" Ivan"});
  return 0;
}